...
async function DeleteComment(postid,id){
    ...
    const response = await fetch(`http://localhost:3000/post/${postid}/comment/${id}`,
        {
            method : "DELETE",
             headers: {
       'Content-Type': 'application/json',
    ...
    return(
        <div>
            ...
            <ul>
                ...
                return <li key={comment.id}>{comment.text} <Link to={`/post/${post.id}/comment/${comment.id}`}>Edit</Link>
